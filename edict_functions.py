...
model_path_clip = "./stable-diffusion-v2-1-base"
clip_tokenizer = CLIPTokenizer.from_pretrained(model_path_clip, subfolder="tokenizer", local_files_only=True)
clip = CLIPTextModel.from_pretrained(model_path_clip, subfolder="text_encoder", local_files_only=True, torch_dtype=torch.float16)

model_path_diffusion = "./stable-diffusion-v2-1-base"

# Build our SD model（全部 float16 以节省显存、提升速度）
unet = UNet2DConditionModel.from_pretrained(
    model_path_diffusion, subfolder="unet", local_files_only=True, torch_dtype=torch.float16
)
vae = AutoencoderKL.from_pretrained(
    model_path_diffusion, subfolder="vae", local_files_only=True, torch_dtype=torch.float16
)
...
vae.to(device)
clip.to(device)
print("Loaded all models (float16)")
...
def im_to_np(im):
    return np.array(im).astype(np.float32) / 255.0 * 2.0 - 1.0
...
def init_attention_func():
    def new_attention(self, query, key, value, sequence_length, dim):
        ...
        for i in range(hidden_states.shape[0] // slice_size):
            ...
            value_slice = value[start_idx:end_idx].to(attn_slice.dtype)
            attn_slice = torch.einsum("b i j, b j d -> b i d", attn_slice, value_slice)
...
def baseline_stablediffusion(prompt="",
    ...
    if init_image is not None:
        ...
        # Move image to GPU and ensure dtype matches VAE
        init_image = init_image.to(device, dtype=vae.dtype)
    ...
    if prev_image is not None:
        ...
        prev_image = np.array(prev_image).astype(np.float32) / 255.0 * 2.0 - 1.0
        ...
        # Move image to GPU and ensure dtype matches VAE
        prev_image = prev_image.to(device, dtype=vae.dtype)
    ...
    with autocast(device):
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps)):
            ...
            if prev_latent is not None:
                prev_noise_pred = prev_noise_pred_uncond + guidance_scale * (
                            prev_noise_pred_cond - prev_noise_pred_uncond)
...
def coupled_stablediffusion(prompt="",
    ...
    def image_to_latent(im):
        if isinstance(im, torch.Tensor):
            ...
            init_latent = im.to(device, dtype=vae.dtype)
        else:
            ...
            # Move image to GPU and ensure dtype matches VAE
            im = im.to(device, dtype=vae.dtype)
            # Encode image
    ...
    if isinstance(latent, list):  # initializing from pair of images
        latent_pair = [l.to(device=device, dtype=unet.dtype) for l in latent]
    else:  # initializing from noise
        latent_pair = [latent.clone().to(device=device, dtype=unet.dtype),
                      latent.clone().to(device=device, dtype=unet.dtype)]
    init_noise = latent
    ...
    embedding_unconditional = clip(tokens_unconditional.input_ids.to(device)).last_hidden_state
    # Ensure embedding dtype matches UNet dtype for cross-attention
    embedding_unconditional = embedding_unconditional.to(dtype=unet.dtype)
    ...
    embedding_conditional = clip(tokens_conditional.input_ids.to(device)).last_hidden_state
    # Ensure embedding dtype matches UNet dtype for cross-attention
    embedding_conditional = embedding_conditional.to(dtype=unet.dtype)
    ...
    if prompt_edit is not None:
        ...
        embedding_conditional_edit = clip(tokens_conditional_edit.input_ids.to(device)).last_hidden_state
        # Ensure embedding dtype matches UNet dtype for cross-attention
        embedding_conditional_edit = embedding_conditional_edit.to(dtype=unet.dtype)
    ...
    if reverse or return_latents:
        results = [latent_pair]
        return results if len(results) > 1 else results[0], init_latent
    ...
    for latent_i in range(2):
        ...
        if binary_arr is not None:
            latent_modify = utils.insert_bit(latent, binary_arr)
        else:
            latent_modify = latent
        if latent_i == 0:
            latent_before_decode = latent_modify

        # decode前的latent
        latents.append(latent)
        # decoder生成图像 - 使用原始latent（matching reference implementation）
        image = vae.decode(latent.to(vae.dtype)).sample
        images.append(image)
    ...
    results = [return_arr]
    return results if len(results) > 1 else results[0], init_noise, latents[0]
